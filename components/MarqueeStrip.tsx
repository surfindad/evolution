...
export default function MarqueeStrip() {
  ...
  return (
    <section className="py-28 bg-[#16161F] overflow-hidden">
      <div className="max-w-7xl mx-auto px-6 lg:px-10 mb-16">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            ...
            <h2 className="font-raleway font-black text-5xl md:text-6xl uppercase gradient-text mb-4">
              Our Portfolio
            </h2>
      ...
      <div className="marquee-container relative mb-4">
        <div className="absolute left-0 top-0 bottom-0 w-40 bg-gradient-to-r from-[#16161F] to-transparent z-10 pointer-events-none" />
        <div className="absolute right-0 top-0 bottom-0 w-40 bg-gradient-to-l from-[#16161F] to-transparent z-10 pointer-events-none" />
          ...
          {doubled.map((company, i) => (
            <div key={`fwd-${company.id}-${i}`} className="flex items-center">
              <Link
                ...
                className="font-raleway font-bold text-2xl md:text-3xl text-white/50 hover:text-green transition-colors duration-300 px-8 whitespace-nowrap"
              >
                {company.name}
              </Link>
        ...
        <div className="marquee-track marquee-track-rev">
          {doubled.map((company, i) => (
            <div key={`rev-${company.id}-${i}`} className="flex items-center">
              <Link
                href={`/portfolio/${company.slug}`}
                className="font-raleway font-bold text-xl md:text-2xl text-white/35 hover:text-green/80 transition-colors duration-300 px-8 whitespace-nowrap"
              >
                {company.name}
              </Link>
